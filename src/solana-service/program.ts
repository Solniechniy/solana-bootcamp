...
export class EscrowProgram {
    ...
    async makeOffer(tokenMintA: PublicKey, tokenMintB: PublicKey, tokenAmountA: number, tokenAmountB: number) {
        ...
        const txInstruction = await this.program.methods.makeOffer(
            offerId, new BN(tokenAmountA), new BN(tokenAmountB)
        ).accounts({ ...accounts, tokenProgram: TOKEN_PROGRAM }).instruction();
    ...
    async takeOffer(maker: PublicKey, offer: PublicKey, tokenMintA: PublicKey, tokenMintB: PublicKey) {
        ...
        console.log(Object.entries(accounts).map(([key, value]) => [key, value.toBase58()]))
        const txInstruction = await this.program.methods.takeOffer().accounts({
            ...accounts
        }).instruction()
